    ...
    for(var i = 0; i < links.length; i++)
    {
        ...
        if(href !== null && href.length > 1 && href.indexOf('#') != -1) // href.substr(0, 1) == '#'
        {
            links[i].onclick = function()
            {
                ...
                if (this.attributes.close) {
                    var closeMunu = this.attributes.close.nodeValue.toString();
                    var u = closeMunu.substr(0, closeMunu.indexOf('#'));
                    var uid = closeMunu.substr(closeMunu.indexOf('#') + 1);
                    document.getElementById(uid).classList.remove('is-active');
                    document.querySelector('div[data-target="'+ uid +'"]').classList.remove('is-active');
                }
